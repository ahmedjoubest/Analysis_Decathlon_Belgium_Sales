...
Generate_random_data <- function(Nb_store, Nb_items, Nb_jours, mean_Nb_Transactions,
  ...
  DT = data.table(
    ...
    tdt_type_detail = sample(c("sale","return"),
                             size = Nb_Transactions,
                             replace = TRUE, # sampling be with replacement
                             prob = c(.85,.15) # assuming that 15% of transactions have items returned
    )
  )
  ...
  Dates <- seq(from = as.Date("2015-01-01"), # Historic data starting from 2015-01-01
  ...
  # Correct the weekday level of Lubridate
  DT[,day:= factor(day,levels = c("Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"))]
  print("day/month/year/quarter added")
  
  ### Add aggregated weeks (for chronological heatmaps)
  # => Attribute for every day the "Monday" date of the week it's a part
  DT[,aggregated_date_week:=as.Date(
    case_when(
      day=="Mon" ~ ymd(the_date_transaction),
      day=="Tue" ~ ymd(the_date_transaction-1),
      day=="Wed" ~ ymd(the_date_transaction-2),
      day=="Thu" ~ ymd(the_date_transaction-3),
      day=="Fri" ~ ymd(the_date_transaction-4),
      day=="Sat" ~ ymd(the_date_transaction-5),
      day=="Sun" ~ ymd(the_date_transaction-6)
    ))]
